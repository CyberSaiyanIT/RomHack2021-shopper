...
Original code by rtphokie
https://github.com/rtphokie/perseverance_parachute_decode

Slightly modified to generate #RomHack2021 shopper graphic
https://romhack.io/shopper-2021.html

syntax: python3 create_romhack.py word1 word2 word3 [word4|coordinates]

where coordinates are of the format deg min sec [N|S] deg min sec [E|W], eg:
34 11 58 N 118 10 31 W

example: $ python3 create_romhack.py '#RomHack2021' romhack.io /shopper 41 50 4 N 12 28 11 E
'''
...
def encode_8_bit_binary(message, colors=[mcolors.to_rgba('#FFF'),mcolors.to_rgba('#F00')]):
    '''
    ...
    for character in encode_int(message):
        result_str +=  f"{character:08b}" # build out binary string
    ...
    result_str+="1"*((words*8)-len(result_str))  # pad remaining gores with 1's
    print(result_str)
...
def encode_int(message, limit=12):
    '''
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='design a JPL parachute for #RomHack2021 shopper')

    parser.add_argument('message', action='store', nargs='*',
                        default = ['#RomHack2021', 'romhack.io', '/shopper','41','50','4','N','12','28','11','E'],
    ...
    parser.add_argument('-red', action='store', default='#ec0b62', help='hex color for red panels (default #F00)')
    ...
    spacing = [(0.6, 0.2), (1.0, 0.4), (1.2, 0.25), (1.5, 0.18)] # (radius, width) matches Perseverance parachute porportions pretty well.
    ...
    for ring,radius_width in zip(message, spacing):
        ...
        panels, last_panel = encode_8_bit_binary(ring, colors=[args.white, args.red])
        radius, width = radius_width
        plot_ring(panels, radius, width, rotate_clockwise_degrees=start_degrees)
        start_degrees+=((last_panel*360)/96)
        i += 1
        

    filename = "romhack2021.png"
    plt.savefig(filename)
    print(f"saved as {filename}")

    filename = "romhack2021.svg"
    plt.savefig(filename)
    print(f"saved as {filename}")
